/*
  # Update city coordinates with additional cities

  1. Changes
    - Updates coordinates for all Polish cities including previously missing ones
    - Adds coordinates for additional cities like Bielsko-Biała, Bytom, etc.
    - Uses COALESCE to handle any unmatched cities
    - Sets default coordinates to Poland's center

  2. Notes
    - Includes more cities than previous migrations
    - Maintains backwards compatibility with existing data
    - Uses proper coordinate precision
*/

-- Update coordinates for all cities including new ones
UPDATE cities SET 
  latitude = COALESCE(
    CASE name
