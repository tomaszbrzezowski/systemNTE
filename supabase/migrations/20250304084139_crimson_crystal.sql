/*
  ...
  1. Changes
    ...
    - Update coordinates for existing cities
    - Make coordinates required for future cities
    
  2. Implementation Details
    - Adds columns as nullable first
    - Updates coordinates for each city
    - Makes columns NOT NULL only for future entries
*/
...
WHERE name IN (
  ...
  'Radom', 'Rzeszów', 'Toruń', 'Kielce', 'Gliwice', 'Zabrze', 'Olsztyn',
  'Zielona Góra', 'Opole', 'Gorzów Wielkopolski', 'Płock', 'Wałbrzych',
  'Tarnów', 'Koszalin', 'Kalisz', 'Legnica', 'Grudziądz', 'Słupsk',
  'Zamość', 'Chełm', 'Piotrków Trybunalski', 'Sieradz', 'Nowy Sącz',
  'Siedlce', 'Nysa', 'Przemyśl', 'Krosno', 'Łomża', 'Suwałki',
  'Ostrowiec Świętokrzyski', 'Elbląg', 'Konin', 'Świnoujście'
);

-- Add default coordinates for any cities that weren't updated
UPDATE cities SET
  latitude = 52.0692,
  longitude = 19.4803
WHERE latitude IS NULL OR longitude IS NULL;

-- Add NOT NULL constraint for future entries only
ALTER TABLE cities 
ALTER COLUMN latitude SET DEFAULT 52.0692,
ALTER COLUMN longitude SET DEFAULT 19.4803;