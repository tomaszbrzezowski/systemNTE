/*
  ...
  1. Changes
    - Updates coordinates for all Polish cities in the database
    - Uses a safer approach with default coordinates for unmatched cities
    - Ensures all cities have valid coordinates before setting NOT NULL constraint

  2. Notes
    - Uses COALESCE to handle cities not in the list
    - Sets default coordinates to Poland's center (52.0692, 19.4803)
*/
...
-- Update coordinates in a transaction
DO $$
BEGIN
  ...
  UPDATE cities SET 
    latitude = COALESCE(
      ...
      END,
      52.0692  -- Default latitude for Poland's center
    ),
    longitude = COALESCE(
      CASE name
      ...
      END,
      19.4803  -- Default longitude for Poland's center
    );
END $$;

-- Now we can safely add NOT NULL constraints since all cities have coordinates
ALTER TABLE cities 
