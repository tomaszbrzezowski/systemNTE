...
BEGIN
  ...
  LOOP
    ...
    -- Get current counter for this year without aggregates
    WITH latest_number AS (
      SELECT SUBSTRING(agreement_number FROM '\d+')::integer AS num
      FROM agreements 
...
-- Set statement timeout for this session only
